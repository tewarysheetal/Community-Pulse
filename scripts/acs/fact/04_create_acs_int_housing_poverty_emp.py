...
TABLE_CODES = ["s1101", "s1701", "s1901", "s2301"]
...
def measure_col_pattern(table_code: str) -> re.Pattern:
    # Matches ACS estimate/margin columns like:
    # s1101_c01_001e, s1701_c02_015m, etc.
...
def build_select_sql(year: int, table_code: str, available_columns: list[str]) -> str:
    ...
    # keep descriptive keys when available
    ...
    # cast all ACS measure columns to numeric
    measure_columns = get_measure_columns(table_code, available_columns)
    if not measure_columns:
        raise ValueError(
            f"stg_acs_{year}_{table_code}_raw has no detected ACS measure columns for pattern '{table_code}_...e/m'"
        )
...
def build_union_view_sql(table_code: str, conn) -> str:
    view_name = f"{TARGET_SCHEMA}.vw_int_acs_{table_code}_all_years"

    # find columns present in every year so the UNION is well-formed
    per_year_cols = [
        get_table_columns(conn, f"int_acs_{year}_{table_code}", TARGET_SCHEMA)
        for year in YEARS
    ]
    common_cols = [c for c in per_year_cols[0] if all(c in cols for cols in per_year_cols[1:])]
    col_list = ", ".join('"' + c + '"' for c in common_cols)

    union_parts = [
        f"SELECT {col_list} FROM {TARGET_SCHEMA}.int_acs_{year}_{table_code}" for year in YEARS
    ]
    union_sql = "\nUNION ALL\n".join(union_parts)
...
def main() -> None:
    ...
    sql_dir = project_root / "sql" / "acs" / "fact" / "int_housing_poverty_emp"
    ...
    try:
        ...
        for table_code in TABLE_CODES:
            view_sql = build_union_view_sql(table_code, conn)
    ...
    master_sql_file = sql_dir / "00_run_all_int_housing_poverty_emp.sql"
    ...
    summary_file = out_dir / "int_housing_povertyemp_build_summary.csv"
