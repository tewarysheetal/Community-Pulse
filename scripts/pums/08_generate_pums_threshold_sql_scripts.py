...
def sql_backfill_year(year: int) -> str:
    ...
    join alice_illinois_essentials_index i
      on i.year = {year}
    where b.year = 2023
...
def main() -> None:
    ...
    f0 = OUTPUT_DIR / "ALICE"/ "07_create_pums_alice_threshold_tables.sql"
    write_file(f0, sql_create_threshold_tables())
    files.append(f0)

    f2023 = OUTPUT_DIR / "2023" / "08_load_pums_alice_thresholds_2023_exact.sql"
    write_file(f2023, sql_load_2023_exact())
    files.append(f2023)

    for year in [2019, 2021, 2022]:
        fy = OUTPUT_DIR / str(year) / f"08_backfill_pums_alice_thresholds_{year}.sql"
